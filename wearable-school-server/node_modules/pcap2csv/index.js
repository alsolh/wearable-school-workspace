...
var mytimer;
var dataArray = [{}];
var fields=['fileName', 'A', 'B', 'AtoB (bit/s)','BtoA (bit/s)'];
	...
	function processor(cb){	
		if(files[i]){
			if(files[i].split('.')[1]=='pcap'){
				...
				console.log('tshark', ['-nr', files[i], '-z', 'conv,'+tcp_udp, '-q']);
				...
				ts.stdout.on('data', function (data) {
		    		...
		    		while (linearray.length>11){
			    		...
			    		dataArray.push({fileName: separates[separates.length-1].split('.')[0], A:linearray[0], B:linearray[2], 'AtoB (bit/s)':AtoB, 'BtoA (bit/s)':BtoA});
