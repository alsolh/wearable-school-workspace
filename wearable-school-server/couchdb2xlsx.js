...
db.list(params, function(error,body,headers) {
    ...
    for(var i = 0; i < body.rows.length; i++) {
        ...
        if(body.rows[i].doc.logType == 'txLog'){
            ...
            sheettxLog.addRow({id: i+1, txnType: body.rows[i].doc.txnType, logTime:body.rows[i].doc.logTime, sessionId:body.rows[i].doc.sessionId,tizenId:body.rows[i].doc.tizenId,endPoint:body.rows[i].doc.endPoint});
        }
        else if (body.rows[i].doc.logType == 'batteryLog'){
            sheetbatteryLog.addRow({id: i+1, battery: body.rows[i].doc.battery, logTime:body.rows[i].doc.logTime, sessionId:body.rows[i].doc.sessionId,tizenId:body.rows[i].doc.tizenId});
        }
        else if (body.rows[i].doc.logType == 'resourceLog'){
            sheetresourceLog.addRow({id: i+1, cpu: body.rows[i].doc.cpu, mem: body.rows[i].doc.mem, logTime:body.rows[i].doc.logTime, sessionId:body.rows[i].doc.sessionId,tizenId:body.rows[i].doc.tizenId});
        }
    }
